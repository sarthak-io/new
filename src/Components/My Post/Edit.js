...
export default function Edit(props) {
  ...
  useEffect(() => {
    ...
    if (postId && currentUserId) {
      ...
      const postRef = ref(db, `Channels/${currentUserId}/posts/${postId}`);

      onValue(postRef, (snapshot) => {
        const post = snapshot.val();
        if (post) {
          // Update the formData state with the fetched post data
          setFormData((prevFormData) => ({ ...prevFormData, ...post }));
          setPostData(post);
          setPostLoaded(true);
    ...
    setUrlUpload(formData.postImage);
  ...
  const handleSubmit = () => {
    ...
    const db = getDatabase();
    const postCountRef = ref(db, 'Channels/' + currentUserId + '/posts'+ postId);
    get(postCountRef)
      .then((snapshot) => {
        ...
        const uploadFilesToStorage = async () => {
          ...
          const uploadPromises = filesToUpload.map(async (field) => {
            ...
            if (file) {
              try {
                const storageRefPath = `channels/${currentUserId}/posts/${postId}/${field}`;
        ...
        uploadFilesToStorage()
          .then((uploadedFiles) => {
            ...
            const newPostNodeRef = ref(db, 'Channels/' + currentUserId + '/posts/' + postId);
  ...
  <div>
    <div class="desk-my-post-bodymain">
        <img
          ...
          alt=""
          src="./public/Group 65.svg"
        />
    <div class="desk-my-post-inner-content2">
      <div onClick={()=>props.setEditMode("All")} style={{cursor:"pointer",left:"48rem"}} class="desk-my-post-bodymain-content2">
      ...
      <div class="desk-my-post-bodymain-content24">
        ...
        />
        <input type='text' placeholder='Unique Token Id' className='input1 i2' value={formData.tokenid}
          name='tokenid'
          onChange={handleChange}></input>
      ...
      >
        ...
        <textarea name='postDescription'
          onChange={handleChange} defaultValue={formData.description} style={{ background: "none" }} class="metadata-is-additional" placeholder='Enter description here...'>
      ...
      <div class="desk-my-post-bodymain-content28">
        ...
        <input class="desk-create-terms-child" type='checkbox' value={formData.termsChecked} name="termsChecked" ></input>
